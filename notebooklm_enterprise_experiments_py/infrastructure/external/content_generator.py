    ...
    def _build_search_params_prompt(self, user_query: str) -> str:
        ...
        instructions = "\n".join(
            [
                ...
                "",
                "【ルール】",
                "※ Vertex AI Searchでは structData.date を使用する",
                "※ 文字列値はダブルクォート(\")で囲む",
                "",
                "1. 期間指定（例: 1/26〜1/30）の場合:",
                '   filter: "structData.date >= \\"2026-01-26\\" AND '
                'structData.date <= \\"2026-01-30\\""',
                "",
                "2. 「直近」「最新」「最近」の場合:",
                '   order_by: "structData.date desc"',
                "",
                "3. 「古い順」「日付順」の場合:",
                '   order_by: "structData.date asc"',
                "",
                "4. 特定の日付（例: 1/30の）の場合:",
                '   filter: "structData.date = \\"2026-01-30\\""',
                "",
                "5. 「以降」「から」の場合:",
                '   filter: "structData.date >= \\"2026-01-25\\""',
                "",
                "6. 日付指定がない場合:",
                "   filter: null, order_by: null",
                "",
                "7. 年が省略されている場合は現在の年を補完",
                "",
                "【出力形式】",
